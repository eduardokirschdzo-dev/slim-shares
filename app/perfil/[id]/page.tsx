  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // 2. REGISTRO DE EVENTO (Isolado com Try/Catch para não travar a tela)
        try {
          await registrarScan(assetCode, checkpointStr);
        } catch (e) {
          console.error("Erro ao registrar o scan (RLS/Tabela):", e);
        }

        // 3. BUSCA OS DADOS REAIS DE ANALYTICS (Totalmente isolado)
        try {
          const { count, data: eventos, error: analyticsError } = await supabase
          ...
          if (eventos && eventos.length > 0) {
            ...
            else if (diffMinutos < 60) setUltimoAcesso(`há ${diffMinutos} min`);
            else if (diffMinutos < 1440) setUltimoAcesso(`há ${Math.floor(diffMinutos / 60)}h`);
          ...
          setUltimoAcesso('Nenhum acesso'); // Destrava o texto caso a View não exista
        }
  ...
  return (
    <main className="min-h-screen bg-[#000000] text-white flex flex-col items-center py-10 px-6 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10 flex flex-col items-center mt-8">
        {!isEditing ? (
          <>
            {/* Foto de Perfil com Borda e Selo */}
              ...
              <label htmlFor="photo-upload" className="cursor-pointer block">
                <div className="w-28 h-28 rounded-full border-2 border-yellow-600 p-1">
                  <div className="w-full h-full bg-[#111] rounded-full overflow-hidden flex items-center justify-center">
                    {fotoUrl ? <img src={fotoUrl} alt={perfil.nome} className="w-full h-full object-cover" /> : <span className="text-4xl">👤</span>}
                  </div>
                </div>
              </label>
            ...
            {/* Lista de Links (Estilo Premium 2x2 GRID) */}
            <div className="w-full grid grid-cols-2 gap-3 mt-4">
              {[
                ...
                <a key={i} href={btn.href} target="_blank" className="flex flex-col items-center justify-center gap-2 w-full p-6 bg-[#111111] hover:bg-[#1a1a1a] hover:border-yellow-600/50 border border-[#222] text-gray-200 font-medium rounded-3xl transition-all shadow-sm">
                  <svg className="w-8 h-8 fill-current text-white mb-1" viewBox="0 0 24 24">{btn.icon}</svg>
                  <span className="text-sm">{btn.label}</span>
                </a>
              ))}
              
              {/* Links Extras no formato GRID */}
              {perfil.links_extras?.map((link: any, index: number) => (
                <a key={index} href={link.url} target="_blank" className="flex flex-col items-center justify-center gap-2 w-full p-6 bg-[#111111] hover:bg-[#1a1a1a] hover:border-yellow-600/50 border border-[#222] text-gray-200 font-medium rounded-3xl transition-all shadow-sm">
            ...
            {/* Card de Analytics - Efeito Neon/3D */}
            <div className="w-full mt-8 bg-[#0a0a0a] border border-yellow-600/30 rounded-3xl p-5 shadow-[0_0_20px_rgba(202,138,4,0.05)] relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-medium text-gray-300 mt-1">{ultimoAcesso}</p>
                </div>
              </div>
          ...
          <div className="w-full bg-[#0a0a0a] p-6 rounded-3xl border border-[#222] text-left space-y-4 shadow-xl z-20 relative">
            ...
            <div><label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Nome</label>
            <input type="text" value={editNome} onChange={(e) => setEditNome(e.target.value)} className="w-full mt-1 bg-[#111] border border-[#333] rounded-xl p-3 text-white focus:border-yellow-500 outline-none transition-colors" /></div>
            
            <div><label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Bio / Subtítulo</label>
            <input type="text" value={editBio} onChange={(e) => setEditBio(e.target.value)} className="w-full mt-1 bg-[#111] border border-[#333] rounded-xl p-3 text-white focus:border-yellow-500 outline-none transition-colors" /></div>
            
            <button onClick={handleSaveChanges} className="w-full mt-6 py-4 bg-yellow-600 hover:bg-yellow-500 text-black font-bold rounded-xl transition-colors shadow-[0_0_15px_rgba(202,138,4,0.3)]">
              Salvar Alterações
            </button>
          </div>
        )}
